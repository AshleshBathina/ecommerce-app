...
function DashboardPage() {
  const { data: ordersData, statsLoading: ordersLoading } = useQuery({
    queryKey: ["orders"],
    queryFn: ordersApi.getAll,
  })

  const { data: statsData, statsLoading: statsLoading } = useQuery({
  ...
  const statsCards = [
    ...
    {
      name: "Total Orders", value: statsLoading ? "..." : statsData?.data?.totalOrders || 0,
                ...
                <tbody>
                  {recentOrders.map(order => (
                    <tr key={order._id} className="hover">
                      ...
                      <td>
                        <span className="font-semibold">₹{orders.totalPrice.toFixed(2)}</span>
                      </td>
                      ...
                      <td>
                        <span className="text-sm opacity-50">{formatDate(order.createdAt)}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
